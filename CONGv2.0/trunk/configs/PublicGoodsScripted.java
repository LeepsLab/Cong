...
public class PublicGoodsScripted implements PayoffScriptInterface, MouseListener, KeyListener {
    ...
    /*
     * public fields of tick are automatically translated to lines in the csv
     * log
     *
     * Both primitive types (String, int, float, boolean) and arrays of
    ...
    public static class Tick implements LogEvent {
        ...
        public int subject;
        public float sum, mean;
        public float action;
        public float payoff;
        // for example, this would give columns named a0, a1, a2
        // public float[] a = new float[3];
        private PublicGoodsScripted weaklink = new PublicGoodsScripted();
        ...
        public void log(
            ...
            for (int id : strategies.keySet()) {
                sum += strategies.get(id)[0];
            }
            mean = sum / strategies.keySet().size();
            action = strategies.get(subject)[0];
            float t = ((config.length * 1000f) - millisLeft) / (config.length * 1000f);
			
            weaklink.config = config;
            //error = weaklink.getError(t);
            //int flipped = Math.round(weaklink.getFlip(t) * strategies.size());
            //flip = -1;
            //for (int id : strategies.keySet()) {
            //   if (id == flipped) {
            //       flip = id;
            //   }
            //}
            payoff = weaklink.getPayoff(subject, t, strategies, matchStrategies, config);
