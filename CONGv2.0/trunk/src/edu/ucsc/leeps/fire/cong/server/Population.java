...
/**
 * This class handles the different groups of clients and helps to distribute 
 * server calls to the clients they refer to.  Among other things it distributes
 * strategy changes to clients in the same group at the the group that is that group's
 * match.  This object is part of the server and is local to the server's JVM.
 * @author jpettit
 */
public class Population implements Serializable {
    
    /**all of the current clients registered with the server*/
    public Map<Integer, ClientInterface> members;
    ...
    /**
     * Configures the population at the beginning of a period.
     * @param members
     * @param aliases
     * @param colors 
     */
    ...
    /**
     * Resets the period start time in the population and updates the groups.
     */
    ...
    /**
     * Passes the strategy change to the correct tuple.
     * @param whoChanged
     * @param newStrategy
    ...
    /**
     * Evaluates all of the current payoffs
     * @param timestamp 
     */
    ...
    /**
     * Ends the current sub-period.  Shuffles groups if sub-period rematch is true
     * and realizes the strategies for probability payoffs.
     * @param subperiod 
     */
    ...
    /**
     * tells the tuple to end the period
     */ 
    ...
    /**
     * Adds an entry to the log using either the scripted payoff function or a
     * default log event.  This is where the log event is instantiated.  The log
     * event is either defined in a payoff function script or the event defaults
     * to a tick event instance.
     * @param subperiod the current subperiod
     * @param millisLeft the amount of time left in the period in milliseconds
     */
    ...
    /**
     * This class holds and manages the client groups.
     */
    private class Tuple {
        int turn=1;
        boolean diceRoll = FIRE.server.getConfig().diceRoll;
        ...
        /**
         * Updates all of the strategies for a group and its match. 
         * @param whoChanged id of the client that changed its strategy
         * @param timestamp timestamp of the strategy change
         */
        ...
        /**
         * Evaluates the payoffs for each client in this tuple.
         * @param t2 the timestamp or sub-period number
         */
        public void evaluate(long t2) {
            ...
            for (int member : members) {
                ...
                final boolean homotopy;
                if (config.get("homotopy") > 0)
                    homotopy = true;
                else
                    homotopy = false;
        ...
        /**
         * Realizes probability payoff strategies. Called at the end of a sub-period.
         */
        ...
        /**
         * Calls evaluate with the number of the sub-period passed
         * @param subperiod 
         */
        ...
        /**
         * Ends the sub-period for all of the clients.  Calculates and passes a 
         * turn number to each of the clients as well as each client's sub-period
         * payoff.
         * @param subperiod 
         */
        public void endSubperiod(final int subperiod) {
            ...
            for (final int member : members) {
                new Thread() {
                    int turnId = (Integer)members.toArray()[turn];
                    @Override
                    public void run() {
                        Config config = FIRE.server.getConfig(member);
                        if(!config.turnBased) {turnId = member;}
                        ...
                        Population.this.members.get(member).endSubperiod(
                                subperiod, strategies, match.strategies, payoff, matchPayoff,turnId,population);
                    }
                }.start();
            }
            update(-1, subperiod);
            ...
            //update turn
            if(diceRoll){
                turn = (int)Math.round(Math.random()*(double)(members.size()-1));
            }else{
                turn++;
        ...
        /**
         * Ends the current period.  Calls evaluate and the ends the period for 
         * the StrategyUpdateProcessors.
         */
        public void endPeriod() {
            if (FIRE.server.getConfig().subperiods == 0) {
    ...
    /**
     * Sets up the groups based off of the config.
     */
    ...
    /**
     * Passes a new message to all of the clients in the group of the messages sender
     * @param secondsLeft seconds left in the period
     * @param message the message
     * @param html the html string for the message
    ...
    /**
     * Convenience class for storing the information for a strategy change to be
    ...
    /**
     * This thread takes strategy updates and passes them to a client as fast as
     * it can. New strategy updates are put into a blocking queue and events are
     * dropped if the queue's size exceeds 10 events.
     */
    private class StrategyUpdateProcessor extends Thread {
        ...
        /**
         * Adds a StrategyUpdateEvent to the blocking queue
         * @param event 
         */
        ...
        /**
         * Clears the queue and resets the number of dropped events
         */
        public void endPeriod() {
