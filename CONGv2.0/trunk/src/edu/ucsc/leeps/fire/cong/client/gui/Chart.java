...
/**
 * Sprite for drawing a chart.  The chart draws lines for each of the client's 
 * strategies.
 * @author jpettit
 */
public class Chart extends Sprite implements Configurable<Config> {
    ...
    private Line averagePayoff;
    private Line topPayoff;
    private Line yourStrategy;
    private Line matchStrategy;
    private Line averageStrategy;
    private Line maxStrategy;
    private Line yourR;
    private Line matchR;
    private Line yourP;
    private Line matchP;
    ...
    /**
     * Enumerated describing different modes for the chart depending on what sort of 
     * strategy is being used.  The mode determines how the chart is drawn and what
     * values it looks for when the lines are updated.
     */
    public enum Mode {
    ...
    /**
     * Constructor
     * @param parent a possible parent sprite
     * @param x x-coordinate relative to the parent if the parent is not null
     * @param y y-coordinate relative to the parent if the parent is not null
     * @param width width of the chart
     * @param height height of the chart
     * @param simplex
     * @param mode a value from the Mode enumerated type to determine how this chart
     * will behave
     */
    public Chart(Sprite parent, int x, int y, int width, int height, ThreeStrategySelector simplex, Mode mode) {
        ...
        scaledMargin = Math.round((height - scaledHeight) / 5f);

        yourPayoff = new Line(this, 0, scaledMargin, width, scaledHeight);
        matchPayoff = new Line(this, 0, scaledMargin, width, scaledHeight);
        averagePayoff = new Line(this, 0, scaledMargin, width, scaledHeight);
        topPayoff = new Line(this, 0, scaledMargin, width, scaledHeight);
        ...
        averageStrategy = new Line(this, 0, scaledMargin, width, scaledHeight);
        maxStrategy = new Line(this, 0, scaledMargin, width, scaledHeight);
    ...
    private void drawAxis(Client applet) {
        ...
        if (mode == Mode.Payoff) {
            ...
            for (float y = 0.0f; y <= 1.01f; y += 0.111111f) {
                applet.noFill();
                applet.stroke(100, 100, 100);
                applet.strokeWeight(2);
                float x0, y0, x1, y1;
                ...
                applet.fill(0);
                float payoff = (Math.round(((1 - y) * (maxPayoff - minPayoff) + minPayoff)*10f))/10f;
                if (payoff < 0) {
                    payoff = 0f;
                }
    ...
    public void draw(Client applet) {
        ...
        if (config.payoffFunction.getNumStrategies() <= 2) {
            if (mode == Mode.Payoff) {
                ...
                /*yourPayoff.r = 25;//Config.colors[0].getRed();
                yourPayoff.g = 220;//Config.colors[0].getGreen();
                yourPayoff.b = 90;//Config.colors[0].getBlue();*/
                ...
                if (config.payoffFunction instanceof PricingPayoffFunction) { //payoff function dependent
                    ...
                    if (config.payoffFunction instanceof QWERTYPayoffFunction == false) {
                        ...
                        yourPayoff.configure( 15,  195, 60,  55,  3f, Line.Mode.Shaded );
                        yourPayoff.draw(applet);
                        
                        if(config.get("drawTop") != 0 || config.get("drawTop") != Float.NaN)
                        {
                            topPayoff.configure( 55,  5, 5,  205,  5f, Line.Mode.Dashed );
                            topPayoff.draw(applet);
                        }
                        if(config.get("drawAverage") != 0)
                        {
                            //averagePayoff.configure( 25,  155, 45,  255,  255.0f, Line.Mode.Solid );
                            //averagePayoff.draw(applet);
                            matchPayoff.configure( 25,  45, 135,  225,  2f, Line.Mode.Solid );
            ...
            if (mode == Mode.Payoff) {
                ...
                if(config.get("drawTop") != 0 || config.get("drawTop") != Float.NaN)
                        {
                            topPayoff.configure( 125,  15, 45,  255,  255.0f, Line.Mode.Solid );
                            topPayoff.draw(applet);
                        }
                        if(config.get("drawAverage") != 0 || config.get("drawTop") != Float.NaN)
                        {
                            averagePayoff.configure( 25,  155, 45,  255,  255.0f, Line.Mode.Solid );
                            averagePayoff.draw(applet);
                        }
    ...
    /**
     * Gets the current percent from the client and update the lines with it.
     */
    ...
    /**
     * Adds a new payoff to this chart's payoff and match payoff lines.
     * @param percent the payoff percent
     */
    public void updateLines(float percent) {
        ...
        if (mode == Mode.Payoff) {
            ...
            if (FIRE.client.getConfig().payoffFunction instanceof PricingPayoffFunction) { //payoff function dependent
                ...
                /*float[] averages = PayoffUtils.getAverageStrategy(Client.state.id, Client.state.matchStrategies);
                float average = 0;
                ...
                averagePayoff.addPayoffPoint(percent, average);
                */
                ...
                {
                    temp = FIRE.client.getConfig().counterpartPayoffFunction.getPayoff(
                        matchID,
                ...
                topPayoff.addPayoffPoint(percent, max);
    ...
    /**
     * Adds a new point to the chars marginal cost line for the current client.
     * @param percent the new payoff percent
     */
